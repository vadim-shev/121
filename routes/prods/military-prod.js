...
export default {
        ...
        justify-content: space-between; position: absolute; "
                            > 
                                 <router-link
                                    ...
                                    :to="'/catalog/military/' + Math.abs(Number($route.params.id) - 1)"
                                >
    ...
    mounted() {
        this.fetchProduct('./data/military.json', 'military'); // Загружаем данные при монтировании
    },
    watch: {
        '$route.params.id': function () {
            this.fetchProduct('./data/military.json', 'military'); // Загружаем данные при изменении ID в маршруте
        },
    },
};
